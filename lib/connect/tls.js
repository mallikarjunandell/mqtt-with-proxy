...
async function buildBuilder (mqttClient, opts) {
  ...
  return new Promise( (resolve, reject) => {
    ...
    if(opts.proxy)
    {
      ...
      modifiedOpts.protocol = 'https:';
      try
      {
        let req = https.request({
          ...modifiedOpts,
          method: 'POST',
          agent: getCaAgent(modifiedOpts, proxy)
        });
        ...
        req.on('socket', (socket) => {
          ...
          opts.hostname = null;
          opts.port = null;
          console.log('Opts.servername is :' + opts.servername);
          
          let proxySocket  = tls.connect(opts);

          proxySocket.on('secureConnect',  () => {
            if (opts.rejectUnauthorized && !proxySocket.authorized) {
              proxySocket.emit('error', new Error('TLS not authorized'))
            } else {
              //proxySocket.removeListener('error', handleTLSerrors)
            }
          });

          proxySocket.on('error', (proxySocketErr) => {
            mqttClient.emit('error', proxySocketErr);
          });
