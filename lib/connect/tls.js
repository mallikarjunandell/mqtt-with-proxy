...
async function buildBuilder (mqttClient, opts) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(opts.proxy)
    {
      ...
      try
      {
        await createSocket(opts, mqttClient).then( proxySocket => {
          let options = opts;
    ...
    try
    {
      ...
      function onConnect(res, socket, head) {
        ...
        socket.on('secureConnect',  () => {
          if (opts.rejectUnauthorized && !socket.authorized) {
            socket.emit('error', 'NOT_AUTHORIZED');
          } else {
            socket.removeListener('error', handleTLSerrors2)
          }
        });

        function handleTLSerrors2 (err) {
          // How can I get verify this error is a tls error?
          if (opts.rejectUnauthorized) {
            mqttClient.emit('error', err)
          }
        ...
        socket.on('error', handleTLSerrors2);
