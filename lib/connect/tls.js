...
const streamSocket = require('stream');
...
async function buildBuilder (mqttClient, opts) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(opts.proxy)
    {
      ...
      try
      {
        await createSocket(opts).then( proxySocket => {
          let options = opts;
          ...
          secureSocket.on('secureConnect',  () => {
            if (opts.rejectUnauthorized && !secureSocket.authorized) {
              let proxySocketErr = new Error('NOT_AUTHORIZED');
              proxySocketErr.code = 'NOT_AUTHORIZED';
              secureSocket.emit('error', proxySocketErr);
            } else {
        ...
        .catch(e => {
          let properErr = new Error('NOT_AUTHORIZED');
          properErr.code = 'NOT_AUTHORIZED';
          mqttClient.emit('error', properErr);
        })
    ...
    else
    {
      ...
      connection.on('secureConnect', function () {
        if (opts.rejectUnauthorized && !connection.authorized) {
          let connErr = new Error('NOT_AUTHORIZED');
          connErr.code = 'NOT_AUTHORIZED';
          connection.emit('error', connErr);
        } else {
