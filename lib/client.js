...
MqttClient.prototype._setupStream = async function () {
  ...
  debug('[MQTT] _setupStream :: using streamBuilder provided to client to create stream')
  this.stream = await this.streamBuilder(this);
  debug('Received stream from protocol :');
  debug(this.stream);
 ...
 * @example client.publish('topic', 'message', debug);
 */
 ...
 * @example client.subscribe({'topic': {qos: 0}, 'topic2': {qos: 1}}, debug);
 * @example client.subscribe('topic', debug);
 */
 ...
 * @example client.unsubscribe('topic', debug);
 */
 ...
 */
MqttClient.prototype._setupReconnect = async function () {
  var that = this
  ...
  if (!that.disconnecting && !that.reconnectTimer && (that.options.reconnectPeriod > 0)) {
    ...
    that.reconnectTimer = setInterval(async function () {
      debug('reconnectTimer :: reconnect triggered!')
      await that._reconnect()
